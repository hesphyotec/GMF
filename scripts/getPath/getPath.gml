		...
		if (isValidPos(aNode.gx, aNode.gy - 1)){
			...
			if (nNode.isFree()){
				if (nNode.gx == endNode.gx && nNode.gy == endNode.gy){
					...
					break;
				} else if (ds_grid_get(closed, nNode.gx, nNode.gy) == false && nNode.isFree()){
					gNew = aNode.g + 1;
		...
		if (isValidPos(aNode.gx, aNode.gy + 1)){
			...
			if (sNode.isFree()){
				if (sNode.gx == endNode.gx && sNode.gy == endNode.gy){
					...
					//show_message("Dest found!");
					break;
				} else if (ds_grid_get(closed, sNode.gx, sNode.gy) == false && sNode.isFree()){
					gNew = aNode.g + 1;
		...
		if (isValidPos(aNode.gx - 1, aNode.gy)){
			...
			if (eNode.isFree()){
				if (eNode.gx == endNode.gx && eNode.gy == endNode.gy){
					...
					//show_message("Dest found!");
					break;
				} else if (ds_grid_get(closed, eNode.gx, eNode.gy) == false && eNode.isFree()){
					gNew = aNode.g + 1;
		...
		if (isValidPos(aNode.gx + 1, aNode.gy)){
			...
			if (wNode.isFree()){
				if (wNode.gx == endNode.gx && wNode.gy == endNode.gy){
					...
					//show_message("Dest found!");
					break;
				} else if (ds_grid_get(closed, wNode.gx, wNode.gy) == false && wNode.isFree()){
					gNew = aNode.g + 1;
...
function gridNode(xx, yy) constructor {
		gx = xx;
		gy = yy;
		px = xx * TILE_SIZE;
		py = yy * TILE_SIZE;
...
function getManDist(nodeA, nodeB){
	return abs(nodeA.gx - nodeB.gx) + abs(nodeA.gy - nodeB.gy);
}

function isValidPos(xx, yy){
	return (xx >= 0 && xx <= room_width / TILE_SIZE) && (yy >= 0 && yy <= room_height / TILE_SIZE);	
}